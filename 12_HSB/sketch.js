...
function draw() {
  ...
  if (mediaPipe.landmarks[0]) { // is hand tracking ready?
    ...
    // interactive Hue (HSB) value
    // hueValue = (hueValue + 1) % 360;  // use this line for animated hueValue
    hueValue = map(distance, 0, width, 0, 360);  // map distance to hueValue

    // draw letter
    fill(hueValue, 100, 100); // HSB fill color
