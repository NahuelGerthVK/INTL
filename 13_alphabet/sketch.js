...
function draw() {
  ...
  if (mediaPipe.landmarks[0]) { // is hand tracking ready?
    ...
    let asciiValue = map(distance, 0, 500, 33, 127); // map distance to ascii value
    let letter = String.fromCharCode(asciiValue); // convert ascii value to letter

    // draw letter
    textSize(distance * 0.9);
    text(letter, centerX, centerY);
    pop();
